...
# Open3 tags captured output with the locale's external encoding. GitHub API
# output is UTF-8, so re-tag both streams at the shared capture seam. Do not
# scrub payload bytes here; callers scrub diagnostics only, and malformed JSON
# or UTF-8 must still fail closed.
def capture3_utf8(*)
  stdout, stderr, status = Open3.capture3(*)
  stdout = stdout.dup.force_encoding(Encoding::UTF_8)
  abort "invalid byte sequence in UTF-8" if status.success? && !stdout.valid_encoding?
